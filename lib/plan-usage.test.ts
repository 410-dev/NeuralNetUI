...
test("model weights accept two decimal places",()=>{assert.equal(hasTwoDecimalPlaces(1.75),true);assert.equal(hasTwoDecimalPlaces(0.3),true);assert.equal(hasTwoDecimalPlaces(2.51),true);assert.equal(hasTwoDecimalPlaces(1.755),false);assert.equal(normalizeModelWeight(1.755),1.76);assert.equal(normalizeModelWeight(0),.01);assert.equal(normalizeModelWeight(500),100);assert.equal(normalizeModelWeight("x"),1);assert.equal(formatModelWeight(1.75),"1.75");assert.equal(formatModelWeight(2),"2");assert.equal(formatModelWeight(0.5),"0.5");});

...
export function publicConfig(config: AppConfig, user: AuthUser): PublicConfig {
  ...
  const planRow=!isAdmin(user)&&user.planId?db.prepare("SELECT served_model_ids AS ids FROM plans WHERE id=?").get(user.planId) as {ids:string}|undefined:undefined;
  let planModels:string[]=[];try{planModels=planRow?JSON.parse(planRow.ids):[];}catch{/* malformed plan is treated as unrestricted until an administrator saves it */}
  const planAllows=(model:ModelConfig)=>!planModels.length||planModels.includes(model.id)||planModels.includes(model.sourceModel);

...
import { modelAllowedByPlan } from "./plan-usage";
import { listMcpConnections, mcpEntitlement } from "./mcp";
import { normalizeEnabledTools } from "./enabled-tools";
...
const preferencesSchema = z.object({ sendReasoningToModel: z.boolean(), exportReasoning: z.boolean(), language: z.enum(["en", "ko"]).default("en"), onDemand: z.boolean().default(false), showModelIdentifiers: z.boolean().default(true), renderStrikethrough: z.boolean().default(true), defaultModelId: z.string().min(1).optional(), defaultReasoningPresetId: z.string().min(1).optional(), aliasBaseModelIds: z.record(z.string().min(1), z.string().min(1)).default({}), appearance: appearanceSchema }).default({ sendReasoningToModel: false, exportReasoning: true, language: "en", onDemand: false, showModelIdentifiers: true, renderStrikethrough: true, aliasBaseModelIds: {}, appearance: DEFAULT_APPEARANCE });
...
export function publicConfig(config: AppConfig, user: AuthUser): PublicConfig {
  const rawAliasBases = user.preferences.aliasBaseModelIds && typeof user.preferences.aliasBaseModelIds === "object" && !Array.isArray(user.preferences.aliasBaseModelIds) ? user.preferences.aliasBaseModelIds as Record<string, unknown> : config.preferences.aliasBaseModelIds || {};
  const aliasBaseModelIds = Object.fromEntries(Object.entries(rawAliasBases).filter((entry): entry is [string, string] => typeof entry[1] === "string"));
  const preferences: AppConfig["preferences"] = { ...config.preferences, sendReasoningToModel: typeof user.preferences.sendReasoningToModel === "boolean" ? user.preferences.sendReasoningToModel : config.preferences.sendReasoningToModel, exportReasoning: typeof user.preferences.exportReasoning === "boolean" ? user.preferences.exportReasoning : config.preferences.exportReasoning, language: user.preferences.language === "ko" || user.preferences.language === "en" ? user.preferences.language : config.preferences.language, showModelIdentifiers: typeof user.preferences.showModelIdentifiers === "boolean" ? user.preferences.showModelIdentifiers : config.preferences.showModelIdentifiers, renderStrikethrough: typeof user.preferences.renderStrikethrough === "boolean" ? user.preferences.renderStrikethrough : config.preferences.renderStrikethrough, defaultModelId: typeof user.preferences.defaultModelId === "string" ? user.preferences.defaultModelId : config.preferences.defaultModelId, defaultReasoningPresetId: typeof user.preferences.defaultReasoningPresetId === "string" ? user.preferences.defaultReasoningPresetId : config.preferences.defaultReasoningPresetId, aliasBaseModelIds, appearance: normalizeAppearance((user.preferences.appearance as Partial<typeof DEFAULT_APPEARANCE>) || config.preferences.appearance), enabledTools: normalizeEnabledTools(user.preferences.enabledTools) };
  const superadmin = user.role === "superadmin";
  const planRow=user.planId?db.prepare("SELECT served_model_ids AS ids FROM plans WHERE id=?").get(user.planId) as {ids:string}|undefined:undefined;
  let planModels:string[]=[];try{const parsed=planRow?JSON.parse(planRow.ids):[];planModels=Array.isArray(parsed)?parsed.filter((value):value is string=>typeof value==="string"):[];}catch{/* malformed plans expose no chat models until an administrator saves them */}
  const planAllows=(model:ModelConfig)=>modelAllowedByPlan(planModels,model.id,model.sourceModel);
  const exposeAllModels=isAdmin(user);
  ...
  return { ...config, ...(modelWeights ? { modelWeights } : {}), planModelIds:planModels, harnessSettings, experimental: { ...config.experimental, hostComputerTool: superadmin && config.experimental.hostComputerTool }, hostComputerAvailable: superadmin && isHostComputerAvailable(), loginAppearance: normalizeLoginAppearance(config.loginAppearance), profile: { name: user.displayName }, preferences, models: config.models.filter((model) => canUseModel(model, user)&&(exposeAllModels||planAllows(model))).map((model) => ({ ...model, reasoningPresets: model.reasoningPresets.filter((preset) => visiblePreset(preset, user)) })), connections: config.connections.map((connection) => ({ ...connection, models: connection.models.filter(model=>exposeAllModels||planAllows(model)).map((model) => ({ ...model, reasoningPresets: model.reasoningPresets.filter((preset) => visiblePreset(preset, user)) })), apiKey: "", hasApiKey: Boolean(!connection.clearApiKey && (connection.apiKey || (connection.driver === "openai" && process.env.OPENAI_API_KEY))) })), mcpConnections:listMcpConnections(user.id),mcpEntitlement:mcpEntitlement(user.id), account: { id: user.id, username: user.username, displayName: user.displayName, role: user.role, canAudit:user.canAudit, ...(user.planId ? { planId:user.planId } : {}) } };
}
...
export function inferModel(input: string | Record<string, unknown>, driver: ConnectionDriver = "openai", connectionId?: string): ModelConfig {
  ...
  const imageGeneration = driver === "openai" && /(?:^|[/:])gpt-image-2(?:$|-)/i.test(modelId);
  return normalizeReasoning({ id: modelId, name: friendlyName, sourceModel, description: `${imageGeneration ? "Image" : "Language"} model from ${provider}`, isAlias: false, visible: true, ...capability, apiContextWindowTokens: inferApiContextWindowTokens(record), visionImageMode: "original", visionMaxEdgePixels: 1024, imageGeneration, imageInput: true, connectionId, reasoningPresets: [] });
}

...
test("user storage quota is enforced atomically and generated images are retained",async()=>{
  ...
  const originalBefore=await readFile(importedImage.path);const modelContent=await readUploadModelContent(importedImage.metadata.id,id,toolSettings);const modelImage=modelContent.find(part=>part.type==="image_file");assert.ok(modelImage&&modelImage.type==="image_file");assert.notEqual(modelImage.file_path,importedImage.path);assert.equal(modelImage.mime_type,"image/jpeg");const processedMetadata=await sharp(modelImage.file_path).metadata();assert.ok((processedMetadata.width||0)<=1600);assert.ok((processedMetadata.height||0)<=1600);assert.deepEqual(await readFile(importedImage.path),originalBefore);assert.equal((await sharp(importedImage.path).metadata()).width,2800);

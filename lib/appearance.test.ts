...
test("appearance preferences fall back to defaults and clamp the chunk size", () => {
  assert.deepEqual(normalizeAppearance(undefined), { lmStudioProgress: "both", accentPalette: "blue", accentColor: "#4d7fd8", streamReveal: "instant", streamPacing: "immediate", streamChunkSize: 3, showReasoningNotes: true, showModelWeights: false, reasoningNotes: {}, greetings: {} });
  assert.equal(normalizeAppearance({ showModelWeights: true }).showModelWeights, true);

...
assert.match(css, /\.mcp-connection-list article > div \{[^}]*position:relative[^}]*padding-right:76px/);
assert.match(css, /\.mcp-connection-list article em \{[^}]*position:absolute[^}]*top:50%[^}]*transform:translateY\(-50%\)[^}]*border:1px solid var\(--border-strong\)/);

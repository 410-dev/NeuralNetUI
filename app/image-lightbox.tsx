...
export function ImageLightbox({src,alt,onClose}:{src:string;alt:string;onClose:()=>void}){
  ...
  return createPortal(<div ref={ref} tabIndex={-1} className="image-lightbox" role="dialog" aria-modal="true" aria-label={alt||(ko?"이미지 미리보기":"Image preview")} onClick={onClose}><button onClick={onClose} aria-label={close} title={close}><X size={25}/></button><img src={src} alt={alt} onClick={event=>event.stopPropagation()}/></div>,document.body);
}

...
function ArtifactDialog({artifact,locale,onClose,onSave}:{artifact:ArtifactDocument;locale:Locale;onClose:()=>void;onSave?:(next:ArtifactDocument)=>void}){
  const ref=useModalFocus(onClose),[editing,setEditing]=useState(false),[content,setContent]=useState(artifact.content);
  ...
  return createPortal(<div ref={ref} tabIndex={-1} className="artifact-layer" role="dialog" aria-modal="true" aria-label={artifact.title}><button className="settings-backdrop" tabIndex={-1} onClick={onClose}/><section className="artifact-dialog"><header><div><Code2 size={18}/><span><strong>{artifact.title}</strong><small>{artifact.kind.toUpperCase()}</small></span></div><nav>{artifact.kind!=="html"&&<button className={editing?"active":""} onClick={()=>setEditing(value=>!value)}><PenLine size={15}/>{locale==="ko"?"수정":"Edit"}</button>}<button onClick={download}><Download size={15}/>{locale==="ko"?"다운로드":"Download"}</button><button aria-label={locale==="ko"?"닫기":"Close"} onClick={onClose}><X size={19}/></button></nav></header><div className={`artifact-workspace ${editing?"editing":""}`}>{editing&&artifact.kind!=="html"&&<textarea aria-label={locale==="ko"?"아티팩트 원본":"Artifact source"} value={content} onChange={event=>setContent(event.target.value)}/>}<div className="artifact-preview"><ArtifactBody artifact={draft}/></div></div>{editing&&<footer><button className="save-button" onClick={()=>{onSave?.({...draft,updatedAt:new Date().toISOString()});setEditing(false);}}><Check size={15}/>{locale==="ko"?"변경 저장":"Save changes"}</button></footer>}</section></div>,document.body);
}

...
import { Check, Code2, Download, FileJson, FileSpreadsheet, Maximize2, Minimize2, PenLine, X } from "lucide-react";
...
function ArtifactDialog({artifact,locale,onClose,onSave}:{artifact:ArtifactDocument;locale:Locale;onClose:()=>void;onSave?:(next:ArtifactDocument)=>void}){
  const {ref,close,closing}=useModalTransition(onClose),[editing,setEditing]=useState(false),[mode,setMode]=useState<"preview"|"code">("preview"),[content,setContent]=useState(artifact.content),[maximized,setMaximized]=useState(false);

...
const translations = {
  en: {
    ...
    availableModels: "Available models", manageModels: "Manage models", unloadModel: "Unload loaded model", unloadingModel: "Unloading…", modelUnloaded: "The model was unloaded.", modelUnloadFailed: "Unable to unload the model.", welcome: "What would you like to explore?",
  ...
  ko: {
    ...
    availableModels: "사용 가능한 모델", manageModels: "모델 관리", unloadModel: "로드된 모델 언로드", unloadingModel: "언로드 중…", modelUnloaded: "모델을 언로드했습니다.", modelUnloadFailed: "모델을 언로드하지 못했습니다.", welcome: "무엇을 함께 살펴볼까요?",
...
export default function Home() {
  ...
  function resetWorkspaceForAuthChange() {
    ...
    setSearching(false); setExportOpen(false); setMobileOpen(false);
  }
    ...
    try {
      ...
      const response = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ conversationId: working.id, branchId, assistantMessageId: placeholder.id, revisionGroupId, modelId: working.modelId, reasoningPresetId: working.reasoningPresetId, sendReasoning: options?.sendReasoning ?? sendReasoning,
          tools: options?.tools || { internetSearch: internetSearchEnabled, pageVisit: pageVisitEnabled, browser: browserEnabled, currentTime: currentTimeEnabled, location: locationEnabled, multipleChoice: multipleChoiceEnabled },
  ...
  async function sendMessage(event?: FormEvent) {
    ...
    if (isGenerating) {
      ...
      const queued: QueuedPrompt = { id: uid("user"), content: text, attachments: draftAttachments, modelId: requestModel.id, reasoningPresetId: requestPreset?.id, sendReasoning,
        tools: { internetSearch: internetSearchEnabled, pageVisit: pageVisitEnabled, browser: browserEnabled, currentTime: currentTimeEnabled, location: locationEnabled, multipleChoice: multipleChoiceEnabled } };
  ...
  return (
    <main className={`app-shell ${messages.length ? "chat-active" : "chat-idle"}`}>
      ...
      <aside className={`sidebar ${collapsed ? "collapsed" : ""} ${mobileOpen ? "mobile-visible" : ""}`}>
        ...
        <button className="profile-card" title={c.settings} aria-label={c.settings} onClick={() => { setSettingsOpen(true); setMobileOpen(false); }}><span className="avatar">{config.profile.name.charAt(0).toUpperCase() || "U"}</span><span><strong>{config.profile.name}</strong><small>{c.settingsConnections}</small></span><Settings2 size={17} /></button>
      </aside>
      ...
      <section className="chat-surface">
        ...
        <div className="model-switcher">
          ...
          {modelMenuOpen && <div className="popover model-popover"><div className="popover-heading"><span>{c.availableModels}</span><small>{visibleModels.length}</small></div>{visibleModels.map((model) => <button className="model-option" key={model.id} onClick={() => chooseModel(model)}><span className="selection-dot">{model.id === selectedModel?.id && <Check size={13} />}</span><span><strong>{model.name}</strong>{config.preferences.showModelIdentifiers !== false && <small>{model.sourceModel}</small>}<em>{model.description}</em></span>{model.isAlias && <b>ALIAS</b>}</button>)}<button className="manage-link" onClick={() => { setModelMenuOpen(false); setSettingsOpen(true); }}><Settings2 size={15} /> {c.manageModels}</button><button className="default-choice-action" disabled={!selectedModel || config.preferences.defaultModelId === selectedModel.id} onClick={() => void setDefaultSelection("model")}><Check size={14} />{config.preferences.defaultModelId === selectedModel?.id ? c.defaultModelActive : c.useAsDefault}</button>{canManageInference && <><button className="unload-model-action" disabled={unloadingModel} title={c.unloadModel} onClick={() => void unloadModel()}>{unloadingModel ? <LoaderCircle className="spin" size={14} /> : <Power size={14} />}{unloadingModel ? c.unloadingModel : c.unloadModel}</button>{modelControlNotice && <p className={`model-control-notice ${modelControlNotice.error ? "error" : ""}`} role="status">{modelControlNotice.message}</p>}</>}</div>}
        </div>
...
function Composer(props: { c: CopySet; draft: string; setDraft: (value: string) => void; sendMessage: (event?: FormEvent) => Promise<void>; keyDown: (event: KeyboardEvent<HTMLTextAreaElement>) => void; isGenerating: boolean; queuedPrompts: QueuedPrompt[]; onRemoveQueuedPrompt: (id: string) => void; selectedModel?: ModelConfig; models: ModelConfig[]; selectedPreset?: ReasoningPreset; contextUsedTokens?: number; presetOpen: boolean; setPresetOpen: (value: boolean) => void; setPreset: (id: string) => void; defaultReasoningPresetId?: string; setDefaultReasoning: () => void; sendReasoning: boolean; toggleSendReasoning: (value: boolean) => void; error: string; clearError: () => void; attachments: StoredAttachment[]; maxAttachments: number; uploadingImages: boolean; onFiles: (files: File[]) => void; onRemoveAttachment: (attachment: StoredAttachment) => void; internetSearchEnabled: boolean; setInternetSearchEnabled: (value: boolean) => void; pageVisitEnabled: boolean; setPageVisitEnabled: (value: boolean) => void; browserEnabled: boolean; setBrowserEnabled: (value: boolean) => void; currentTimeEnabled: boolean; setCurrentTimeEnabled: (value: boolean) => void; locationEnabled: boolean; setLocationEnabled: (value: boolean) => void; multipleChoiceEnabled: boolean; setMultipleChoiceEnabled: (value: boolean) => void; pendingChoice?: ToolEvent; onChoiceSubmit: (id: string, value: unknown) => Promise<boolean> }) {
  ...
  const showStop = isGenerating && (!hasDraftMessage || Boolean(pendingChoice));
  return <div className="composer-wrap">
    ...
    <form className="composer" onSubmit={sendMessage}>
      ...
      <textarea aria-label={pendingChoice ? c.choiceWaiting : c.messagePlaceholder} enterKeyHint="enter" rows={draft ? 2 : 1} value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={keyDown} placeholder={pendingChoice ? c.choiceWaiting : c.messagePlaceholder} disabled={Boolean(pendingChoice)} />
      <div className="composer-actions">
        <div className="add-menu-wrap" ref={addMenuRef}>
          ...
          {addMenuOpen && <div className="popover add-menu-popover">
            ...
            <p>{c.tools}</p>
             ...
             <div className="tool-toggle-row"><Monitor size={17} /><span><strong>{c.browserTool}</strong><small>{c.browserToolDesc}</small></span><button type="button" role="switch" aria-label={c.browserTool} aria-checked={browserEnabled} className={`toggle ${browserEnabled ? "on" : ""}`} onClick={() => setBrowserEnabled(!browserEnabled)}><i /></button></div>
        ...
        {(internetSearchEnabled || pageVisitEnabled || browserEnabled || currentTimeEnabled || locationEnabled || multipleChoiceEnabled) && <small className="enabled-tools"><Wrench size={11} />{Number(internetSearchEnabled) + Number(pageVisitEnabled) + Number(browserEnabled) + Number(currentTimeEnabled) + Number(locationEnabled) + Number(multipleChoiceEnabled)}</small>}
...
function ToolEventIcon({ name, size = 15 }: { name: string; size?: number }) {
  const Icon = name === "internet_search" ? Search : name === "visit_page" ? Link2 : name === "browser" ? Monitor : name === "get_current_time" ? Clock3 : name === "get_current_location" ? LocateFixed : name === "ask_multiple_choice" ? ListChecks : Wrench;
...
function ReasoningSettings({ c, draft, setDraft, activeModelId, setActiveModelId, activeModel, updateModel, addPreset, account }: ModelEditorProps & { c: CopySet; setDraft: React.Dispatch<React.SetStateAction<PublicConfig>>; updateModel: (patch: Partial<ModelConfig>) => void; addPreset: () => string; account?: AccountInfo }) {
  ...
  const canEditPreset = (preset: ReasoningPreset) => preset.kind === "custom" && (privileged || ownsModel || preset.ownerId === account?.id);
  const selectedPreset = activeModel?.reasoningPresets.find((preset) => preset.id === activePresetId) || activeModel?.reasoningPresets[0];
  const activePresetIndex = activeModel?.reasoningPresets.findIndex((preset) => preset.id === selectedPreset?.id) ?? -1;
  ...
  function nudgePreset(offset: -1 | 1) { if (!activeModel || !selectedPreset) return; updateModel({ reasoningPresets: nudgeItemById(activeModel.reasoningPresets, selectedPreset.id, offset) }); }
  const presetActions = <OrderActions c={c} onAdd={() => setActivePresetId(addPreset())} addLabel={c.addTemplate} onUp={() => nudgePreset(-1)} onDown={() => nudgePreset(1)} disableUp={activePresetIndex <= 0} disableDown={activePresetIndex < 0 || activePresetIndex === (activeModel?.reasoningPresets.length || 0) - 1} />;
  return <div className="settings-section wide"><SectionTitle icon={<BrainCircuit size={19} />} title={c.reasoningTitle} description={c.reasoningDesc} action={presetActions} /><div className="split-model-editor"><ModelColumn c={c} models={visibleModels} active={activeModelId} onChange={(id) => { setActiveModelId(id); setActivePresetId(draft.models.find((model) => model.id === id)?.reasoningPresets[0]?.id || ""); }} showIdentifiers={draft.preferences.showModelIdentifiers !== false} onMove={moveModel} action={<OrderActions c={c} onUp={() => nudgeModel(-1)} onDown={() => nudgeModel(1)} disableUp={activeModelIndex <= 0} disableDown={activeModelIndex < 0 || activeModelIndex === visibleModels.length - 1} />} /><div className="model-editor-pane reasoning-pane">{activeModel ? <><div className="reason-capability"><div><strong>{c.nativeSupport}</strong><small>{activeModel.reasoningEfforts?.length ? `${hasEffort ? "Effort" : "Toggle"}: ${activeModel.reasoningEfforts.map(reasoningOptionName).join(", ")}` : c.noEffortMetadata}</small></div><button role="switch" aria-label={c.nativeSupport} aria-checked={activeModel.reasoningSupported} disabled={activeModel.isAlias || !privileged || !efforts.length} className={`toggle ${activeModel.reasoningSupported ? "on" : ""}`} onClick={() => updateModel({ reasoningSupported: !activeModel.reasoningSupported })}><i /></button></div><div className="preset-list">{activeModel.reasoningPresets.map((preset) => <div className={`preset-editor ${preset.id === selectedPreset?.id ? "active" : ""}`} key={preset.id} onClick={() => setActivePresetId(preset.id)} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; }} onDrop={(event) => { event.preventDefault(); movePreset(event.dataTransfer.getData("text/plain"), preset.id); }}><div className="preset-editor-head"><span className={`kind-icon ${preset.kind}`} draggable title={c.priorityHelp} onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", preset.id); }}><GripVertical size={15} /></span><input disabled={!canEditPreset(preset)} value={preset.name} onChange={(event) => patchPreset(preset.id, { name: event.target.value })} aria-label={c.reasoningPreset} /><select disabled value={preset.kind} onChange={(event) => patchPreset(preset.id, { kind: event.target.value as ReasoningPreset["kind"] })}><option value="builtin">{c.builtIn}</option><option value="custom">{c.customTemplate}</option></select><button aria-label={`${draft.preferences.language === "ko" ? "추론 프리셋 삭제" : "Delete reasoning preset"}: ${preset.name}`} disabled={!canEditPreset(preset)} onClick={() => removePreset(preset.id)}><Trash2 size={15} /></button></div><div className="preset-editor-body">{activeModel.reasoningSupported && <label><span>{hasEffort ? c.nativeEffort : c.reasoningLevel}</span><select disabled={!canEditPreset(preset)} value={efforts.includes(preset.effort || "") ? preset.effort : ""} onChange={(event) => patchPreset(preset.id, { effort: event.target.value })}><option value="">{c.doNotSend}</option>{efforts.map((effort) => <option value={effort} key={effort}>{reasoningOptionName(effort)}</option>)}</select></label>}{preset.kind === "custom" && <><label><span>{c.promptHandling}</span><select disabled={!canEditPreset(preset)} value={preset.systemPromptMode || "append"} onChange={(event) => patchPreset(preset.id, { systemPromptMode: event.target.value as ReasoningPreset["systemPromptMode"] })}><option value="replace">{c.replace}</option><option value="prepend">{c.prepend}</option><option value="append">{c.append}</option></select></label><label><span>{c.additionalPrompt}</span><textarea disabled={!canEditPreset(preset)} rows={3} value={preset.systemPrompt || ""} onChange={(event) => patchPreset(preset.id, { systemPrompt: event.target.value })} placeholder={c.systemPromptPlaceholder} /></label></>}</div></div>)}{!activeModel.reasoningPresets.length && <EmptyState text={c.noPresets} />}</div></> : <EmptyState text={c.noModel} />}</div></div></div>;
}

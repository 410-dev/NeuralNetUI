...
import { multipleChoiceAnswers, pendingMultipleChoiceEvent, removeUserMessagePair } from "@/lib/conversation-messages";
...
type SettingsTab = "general" | "appearance" | "connection" | "tools" | "experimental" | "models" | "reasoning" | "users" | "account";
...
const emptyConfig: PublicConfig = {
  ...
  userStorageSettings: { defaultQuotaBytes: 512 * 1024 * 1024 },
...
const translations = {
  en: {
    newChat: "New Chat", search: "Search", storageManager: "Storage manager", searchChats: "Search chats…", histories: "Chat histories", exportChat: "Export chat", deleteChat: "Delete chat", deleteAllChats: "Delete all chats", confirmDeleteChat: "Delete this chat permanently?", confirmDeleteAllChats: "Delete all chat histories permanently?",
  ...
  ko: {
    newChat: "새 채팅", search: "검색", storageManager: "저장소 관리", searchChats: "채팅 검색…", histories: "채팅 기록", exportChat: "채팅 내보내기", deleteChat: "대화 삭제", deleteAllChats: "전체 대화 삭제", confirmDeleteChat: "이 대화를 영구적으로 삭제할까요?", confirmDeleteAllChats: "모든 대화 기록을 영구적으로 삭제할까요?",
    ...
    cancel: "취소", close: "닫기", forkSend: "분기 후 전송", editBranch: "편집 후 분기", reasoning: "Reasoning", copy: "복사", regenerate: "응답 재생성", regenerateRequest: "이 메시지부터 재생성", deleteMessage: "메시지 삭제", confirmDeleteMessage: "이 메시지와 연결된 모델 응답을 함께 삭제할까요?", previousRevision: "이전 수정본", nextRevision: "다음 수정본",
...
export default function Home() {
  ...
  const pendingHostApproval = useMemo(() => {
    for (let messageIndex = messages.length - 1; messageIndex >= 0; messageIndex -= 1) {
      ...
      for (let eventIndex = events.length - 1; eventIndex >= 0; eventIndex -= 1) {
        ...
        if (event.name === "host_computer" && event.status === "waiting") return event;
      }
    }
    return undefined;
  }, [messages]);
  ...
  async function sendMessage(event?: FormEvent) {
    ...
    if (isGenerating) {
      ...
      const queued: QueuedPrompt = { id: uid("user"), content: text, attachments: draftAttachments, modelId: requestModel.id, reasoningPresetId: requestPreset?.id, sendReasoning,
        tools: { internetSearch: internetSearchEnabled, pageVisit: pageVisitEnabled, browser: browserEnabled, hostComputer: hostComputerToolAvailable && hostComputerEnabled, currentTime: currentTimeEnabled, location: locationEnabled, multipleChoice: multipleChoiceEnabled } };
  ...
  async function deleteUserMessage(messageId: string) {
    ...
    const stamp = now(); const nextMessages = removeUserMessagePair(source.messages, messageId);
  ...
  async function uploadImages(files: File[]) {
    ...
    const selected = files.filter((file) => file.type === "application/pdf" || file.name.toLowerCase().endsWith(".pdf") || imageTypes.has(file.type)).slice(0, remaining);
    ...
    try {
      ...
      setDraftAttachments((current) => [...current, ...body.attachments].slice(0, config.toolSettings.maxAttachmentsPerMessage));
    ...
    <main className={`app-shell ${messages.length ? "chat-active" : "chat-idle"} ${temporaryActive ? "temporary-chat" : ""} ${browserEnabled && browserViewOpen ? "browser-split-open" : ""}`}>
      ...
      <section className="chat-surface">
        ...
        <div className="conversation-stage">
          {!messages.length ? <div className="idle-center"><div className="welcome"><h1>{temporaryActive ? c.temporaryGreeting : greeting}</h1><p>{temporaryActive ? c.temporaryChatHint : c.welcome}</p></div><Composer c={c} appearance={appearance} draft={draft} setDraft={setDraft} sendMessage={sendMessage} keyDown={handleComposerKeyDown} isGenerating={isGenerating} queuedPrompts={queuedPrompts} onRemoveQueuedPrompt={removeQueuedPrompt} selectedModel={selectedModel} models={config.models} selectedPreset={selectedPreset} contextBreakdown={contextBreakdown} presetOpen={presetMenuOpen} setPresetOpen={setPresetMenuOpen} setPreset={setSelectedPresetId} defaultReasoningPresetId={config.preferences.defaultReasoningPresetId} setDefaultReasoning={() => void setDefaultSelection("reasoning")} admin={isAdmin} applyingReasoningDefault={applyingDefault === "reasoning"} applyReasoningToEveryone={() => void applyDefaultToEveryone("reasoning")} sendReasoning={sendReasoning} toggleSendReasoning={toggleSendReasoning} error={error} clearError={() => setError("")} attachments={draftAttachments} maxAttachments={config.toolSettings.maxAttachmentsPerMessage} uploadingImages={uploadingImages} onFiles={uploadImages} onRemoveAttachment={removeDraftAttachment} internetSearchEnabled={internetSearchEnabled} setInternetSearchEnabled={setInternetSearchEnabled} pageVisitEnabled={pageVisitEnabled} setPageVisitEnabled={setPageVisitEnabled} browserToolAvailable={browserToolAvailable} browserEnabled={browserEnabled} setBrowserEnabled={setBrowserEnabled} hostComputerToolAvailable={hostComputerToolAvailable} hostComputerEnabled={hostComputerEnabled} setHostComputerEnabled={setHostComputerEnabled} currentTimeEnabled={currentTimeEnabled} setCurrentTimeEnabled={setCurrentTimeEnabled} locationEnabled={locationEnabled} setLocationEnabled={setLocationEnabled} multipleChoiceEnabled={multipleChoiceEnabled} setMultipleChoiceEnabled={setMultipleChoiceEnabled} pendingChoice={pendingChoice} pendingHostApproval={pendingHostApproval} onChoiceSubmit={submitToolInput} /></div> : <>
...
function Composer(props: { c: CopySet; appearance: AppearancePreferences; draft: string; setDraft: (value: string) => void; sendMessage: (event?: FormEvent) => Promise<void>; keyDown: (event: KeyboardEvent<HTMLTextAreaElement>) => void; isGenerating: boolean; queuedPrompts: QueuedPrompt[]; onRemoveQueuedPrompt: (id: string) => void; selectedModel?: ModelConfig; models: ModelConfig[]; selectedPreset?: ReasoningPreset; contextBreakdown: ContextUsage; presetOpen: boolean; setPresetOpen: (value: boolean) => void; setPreset: (id: string) => void; defaultReasoningPresetId?: string; setDefaultReasoning: () => void; admin: boolean; applyingReasoningDefault: boolean; applyReasoningToEveryone: () => void; sendReasoning: boolean; toggleSendReasoning: (value: boolean) => void; error: string; clearError: () => void; attachments: StoredAttachment[]; maxAttachments: number; uploadingImages: boolean; onFiles: (files: File[]) => void; onRemoveAttachment: (attachment: StoredAttachment) => void; internetSearchEnabled: boolean; setInternetSearchEnabled: (value: boolean) => void; pageVisitEnabled: boolean; setPageVisitEnabled: (value: boolean) => void; browserToolAvailable: boolean; browserEnabled: boolean; setBrowserEnabled: (value: boolean) => void; hostComputerToolAvailable: boolean; hostComputerEnabled: boolean; setHostComputerEnabled: (value: boolean) => void; currentTimeEnabled: boolean; setCurrentTimeEnabled: (value: boolean) => void; locationEnabled: boolean; setLocationEnabled: (value: boolean) => void; multipleChoiceEnabled: boolean; setMultipleChoiceEnabled: (value: boolean) => void; pendingChoice?: ToolEvent; pendingHostApproval?: ToolEvent; onChoiceSubmit: (id: string, value: unknown) => Promise<boolean> }) {
  ...
  const activeToolCount = Number(internetSearchEnabled) + Number(pageVisitEnabled) + Number(browserToolAvailable && browserEnabled) + Number(hostComputerToolAvailable && hostComputerEnabled) + Number(currentTimeEnabled) + Number(locationEnabled) + Number(multipleChoiceEnabled);
  useEffect(() => {
  ...
  return <div className="composer-wrap">
    ...
    {pendingHostApproval && <HostApprovalComposer key={pendingHostApproval.id} c={c} event={pendingHostApproval} onSubmit={(value) => onChoiceSubmit(pendingHostApproval.id, value)} />}
    <form ref={formRef} className={`composer ${expanded ? "expanded" : ""}`} onSubmit={sendMessage}>
      ...
      <div className="composer-lead" ref={leadRef}>
        <div className="add-menu-wrap" ref={addMenuRef}>
          ...
          {addMenuMounted && <div className={`popover add-menu-popover ${addMenuClosing ? "closing" : ""}`}>
            <button type="button" className="add-menu-action" onClick={() => { fileRef.current?.click(); setAddMenuOpen(false); }} disabled={uploadingImages || attachments.length >= maxAttachments}><ImagePlus size={17} /><span><strong>{c.attachImages}</strong></span></button>
            <p>{c.tools}</p>
             <div className="tool-toggle-row"><Globe2 size={17} /><span><strong>{c.internetSearch}</strong><small>{c.internetSearchDesc}</small></span><button type="button" role="switch" aria-label={c.internetSearch} aria-checked={internetSearchEnabled} className={`toggle ${internetSearchEnabled ? "on" : ""}`} onClick={() => setInternetSearchEnabled(!internetSearchEnabled)}><i /></button></div>
             <div className="tool-toggle-row"><Link2 size={17} /><span><strong>{c.pageVisit}</strong><small>{c.pageVisitDesc}</small></span><button type="button" role="switch" aria-label={c.pageVisit} aria-checked={pageVisitEnabled} className={`toggle ${pageVisitEnabled ? "on" : ""}`} onClick={() => setPageVisitEnabled(!pageVisitEnabled)}><i /></button></div>
             <div className="tool-toggle-row"><Clock3 size={17} /><span><strong>{c.currentTime}</strong><small>{c.currentTimeDesc}</small></span><button type="button" role="switch" aria-label={c.currentTime} aria-checked={currentTimeEnabled} className={`toggle ${currentTimeEnabled ? "on" : ""}`} onClick={() => setCurrentTimeEnabled(!currentTimeEnabled)}><i /></button></div>
             <div className="tool-toggle-row"><MapPin size={17} /><span><strong>{c.locationTool}</strong><small>{c.locationToolDesc}</small></span><button type="button" role="switch" aria-label={c.locationTool} aria-checked={locationEnabled} className={`toggle ${locationEnabled ? "on" : ""}`} onClick={() => setLocationEnabled(!locationEnabled)}><i /></button></div>
              {browserToolAvailable && <div className="tool-toggle-row"><Monitor size={17} /><span><strong>{c.browserTool}</strong><small>{c.browserToolDesc}</small></span><button type="button" role="switch" aria-label={c.browserTool} aria-checked={browserEnabled} className={`toggle ${browserEnabled ? "on" : ""}`} onClick={() => setBrowserEnabled(!browserEnabled)}><i /></button></div>}
              {hostComputerToolAvailable && <div className="tool-toggle-row"><HardDrive size={17} /><span><strong>{c.hostComputerTool}</strong><small>{c.hostComputerToolDesc}</small></span><button type="button" role="switch" aria-label={c.hostComputerTool} aria-checked={hostComputerEnabled} className={`toggle ${hostComputerEnabled ? "on" : ""}`} onClick={() => setHostComputerEnabled(!hostComputerEnabled)}><i /></button></div>}
             <div className="tool-toggle-row"><ListChecks size={17} /><span><strong>{c.multipleChoice}</strong><small>{c.multipleChoiceDesc}</small></span><button type="button" role="switch" aria-label={c.multipleChoice} aria-checked={multipleChoiceEnabled} className={`toggle ${multipleChoiceEnabled ? "on" : ""}`} onClick={() => setMultipleChoiceEnabled(!multipleChoiceEnabled)}><i /></button></div>
          </div>}
        </div>
      ...
      <div className="composer-field">
        ...
        <textarea ref={textRef} aria-label={pendingInput ? c.choiceWaiting : c.messagePlaceholder} enterKeyHint="enter" rows={1} value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={keyDown} onPaste={(event) => {
          ...
          event.preventDefault();
          if (!uploadingImages && !pendingInput) onFiles(images);
...
function ToolEventIcon({ name, size = 15 }: { name: string; size?: number }) {
  const Icon = name === "internet_search" ? Search : name === "visit_page" ? Link2 : name === "browser" ? Monitor : name === "host_computer" ? HardDrive : name === "get_current_time" ? Clock3 : name === "get_current_location" ? LocateFixed : name === "ask_multiple_choice" ? ListChecks : Wrench;
...
function AttachmentGrid({ attachments }: { attachments: StoredAttachment[] }) {
  return <div className={`message-attachments count-${Math.min(attachments.length, 4)}`}>{attachments.map((attachment) => <a className={attachment.mimeType === "application/pdf" ? "pdf-attachment" : ""} href={attachment.url} target="_blank" rel="noreferrer" key={attachment.id} title={attachment.name}>{attachment.thumbnailUrl ? <img src={attachment.thumbnailUrl} alt={attachment.name} loading="lazy" decoding="async" width={attachment.width} height={attachment.height} /> : <span><FileText size={28} /><strong>{attachment.name}</strong><small>{Math.max(.01, attachment.size / 1024 / 1024).toFixed(2)} MB</small></span>}</a>)}</div>;
}
  ...
  async function detect(connectionId: string) {
    ...
    try {
      ...
      const detected: ModelConfig[] = body.models.map((model: ModelConfig) => {
        ...
        return previous ? { ...model, name: previous.name, visible: previous.visible, description: previous.description, systemPrompt: previous.systemPrompt, reasoningPresets: normalizeReasoning({ ...model, reasoningPresets: previous.reasoningPresets }).reasoningPresets, contextWindowTokens: previous.contextWindowTokens } : model;
      });
...
function ConnectionSettings({ c, draft, setDraft, onDetect, detectingConnectionId }: { c: CopySet; draft: PublicConfig; setDraft: React.Dispatch<React.SetStateAction<PublicConfig>>; onDetect: (connectionId: string) => void; detectingConnectionId: string }) {
  ...
  const locale: Locale = draft.preferences.language === "ko" ? "ko" : "en";
  const driverOptions = ([["openai", "OpenAI API"], ["lmstudio", "LM Studio"]] as Array<[ConnectionDriver, string]>).map(([driver, name]) => {
  ...
  return <div className="settings-section wide"><SectionTitle icon={<Server size={19} />} title={c.serverTitle} description={c.serverDesc} />
    <div className="split-model-editor connection-editor">
      ...
      <div className="model-editor-pane">{activeConnection ? <div className="editor-card connection-card">{(() => { const detecting = detectingConnectionId === activeConnection.id; return <>
        ...
        <div className="form-grid"><label className="field"><span>{c.connectionName}</span><input value={activeConnection.name} onChange={(event) => patchConnection(activeConnection.id, { name: event.target.value })} /></label><label className="field"><span>{c.driver}</span><SelectMenu label={c.driver} value={activeConnection.driver} options={driverOptions} onChange={(value) => { const driver = value as "openai" | "lmstudio"; patchConnection(activeConnection.id, { driver, baseUrl: driver === "lmstudio" && activeConnection.baseUrl === "http://localhost:8888/v1" ? "http://localhost:1234" : activeConnection.baseUrl }); }} /></label></div>
...
function ManagedUserDialog({c,user,currentUserId,onUsers,onDeleted,onClose}:{c:CopySet;user:UserSummary;currentUserId:string;onUsers:(users:UserSummary[])=>void;onDeleted:()=>void;onClose:()=>void}){
  const ko=c.users==="사용자";const initialQuota=quotaParts(user.storageQuotaBytes);const[name,setName]=useState(user.displayName);const[role,setRole]=useState(user.role);const[quota,setQuota]=useState(initialQuota.value);const[unit,setUnit]=useState<"MB"|"GB">(initialQuota.unit);const[data,setData]=useState<{conversations:ConversationSummary[];storage:{files:StorageFile[];usedBytes:number;quotaBytes:number}}>();const[notice,setNotice]=useState("");const[busy,setBusy]=useState(false);const ref=useModalFocus(onClose);
  const load=()=>fetch(`/api/users/${encodeURIComponent(user.id)}/audit`,{cache:"no-store"}).then(async response=>{const body=await response.json();if(!response.ok)throw new Error(body.error);setData(body);});
  useEffect(()=>{void load().catch(error=>setNotice(error instanceof Error?error.message:"Unable to load user data."));},[user.id]);
  async function save(){setBusy(true);setNotice("");try{const storageQuotaBytes=Math.round(quota*(unit==="GB"?1024**3:1024**2));const payload=user.id===currentUserId?{storageQuotaBytes}:{displayName:name,role:user.role==="superadmin"?undefined:role,storageQuotaBytes};const response=await fetch(`/api/users/${encodeURIComponent(user.id)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});const body=await response.json();if(!response.ok)throw new Error(body.error);onUsers(body.users||[]);await load();setNotice(c.saved);}catch(error){setNotice(error instanceof Error?error.message:"Unable to update user.");}finally{setBusy(false);}}
  async function remove(){if(!window.confirm(`${c.confirmDeleteUser}\n\n${user.displayName} (@${user.username})`))return;setBusy(true);try{const response=await fetch(`/api/users/${encodeURIComponent(user.id)}`,{method:"DELETE"});const body=await response.json();if(!response.ok)throw new Error(body.error);onUsers(body.users||[]);onDeleted();}catch(error){setNotice(error instanceof Error?error.message:"Unable to delete user.");setBusy(false);}}
  const exportUrl=(kind:string,extra="")=>`/api/users/${encodeURIComponent(user.id)}/audit?download=${kind}${extra}`;
  return createPortal(<div ref={ref} tabIndex={-1} className="harness-modal-layer managed-user-layer" role="dialog" aria-modal="true" aria-label={`${user.displayName} ${c.userManagement}`}><button className="settings-backdrop" tabIndex={-1} onClick={onClose}/><section className="harness-dialog managed-user-dialog"><header><div><h2>{user.displayName}</h2><p>@{user.username} · {user.role}</p></div><button onClick={onClose} aria-label={c.close}><X size={20}/></button></header>
    <div className="managed-user-scroll"><section className="managed-user-section"><h3>{ko?"계정 및 저장소":"Account and storage"}</h3><div className="form-grid"><label className="field"><span>{c.displayName}</span><input disabled={user.id===currentUserId} value={name} onChange={event=>setName(event.target.value)}/></label><label className="field"><span>{c.role}</span>{user.role==="superadmin"||user.id===currentUserId?<input disabled value={user.role}/>:<SelectMenu label={c.role} value={role} options={[{value:"user",label:c.standardUser},{value:"admin",label:c.administrator}]} onChange={value=>setRole(value as "user"|"admin")}/>}</label></div><div className="quota-editor"><label className="field"><span>{ko?"저장소 할당량":"Storage quota"}</span><input type="number" min={1} step={unit==="GB"?.25:1} value={quota} onChange={event=>setQuota(Number(event.target.value))}/></label><SelectMenu label={ko?"용량 단위":"Quota unit"} value={unit} options={[{value:"MB",label:"MB"},{value:"GB",label:"GB"}]} onChange={value=>setUnit(value as "MB"|"GB")}/><small>{ko?`현재 ${storageBytesLabel(data?.storage.usedBytes??user.storageUsedBytes)} 사용 중`:`Currently using ${storageBytesLabel(data?.storage.usedBytes??user.storageUsedBytes)}`}</small></div><button className="save-button" disabled={busy||!name.trim()||quota<=0} onClick={()=>void save()}>{busy?c.saving:c.saveChanges}</button></section>
    <section className="managed-user-section"><header><h3>{ko?"채팅 기록 · 모든 분기 포함":"Chat records · all branches included"}</h3><a className="subtle-action" href={exportUrl("conversations")}><Download size={14}/>{ko?"전체 압축":"Export all"}</a></header><div className="audit-list">{data?.conversations.map(item=><div key={item.id}><span><strong>{item.title}</strong><small>{item.branchCount} {ko?"개 분기":"branches"} · {new Date(item.updatedAt).toLocaleString(ko?"ko-KR":"en-US")}</small></span><a href={exportUrl("conversation",`&conversationId=${encodeURIComponent(item.id)}`)} aria-label={`${item.title} ${ko?"압축":"export"}`}><Download size={15}/></a></div>)}{data&&!data.conversations.length&&<p>{ko?"채팅 기록이 없습니다.":"No chat records."}</p>}</div></section>
    <section className="managed-user-section"><header><h3>{ko?"미디어 파일":"Media files"}</h3><a className="subtle-action" href={exportUrl("media")}><Download size={14}/>{ko?"전체 압축":"Export all"}</a></header><div className="audit-media-grid">{data?.storage.files.map(file=><a key={file.id} href={exportUrl("media-file",`&uploadId=${encodeURIComponent(file.id)}`)} title={file.name}>{file.mimeType.startsWith("image/")?<img src={exportUrl("media-file",`&uploadId=${encodeURIComponent(file.id)}&inline=1`)} alt="" loading="lazy"/>:<FileText size={23}/>}<span><strong>{file.name}</strong><small>{storageBytesLabel(file.size)}</small></span><Download size={14}/></a>)}{data&&!data.storage.files.length&&<p>{ko?"미디어 파일이 없습니다.":"No media files."}</p>}</div></section></div>
    {notice&&<p className="settings-notice" role="status">{notice}</p>}<footer><button className="secondary-button" onClick={onClose}>{c.close}</button>{user.role!=="superadmin"&&user.id!==currentUserId&&<button className="danger-button" disabled={busy} onClick={()=>void remove()}><Trash2 size={14}/>{c.deleteUser}</button>}</footer></section></div>,document.body);
}

function UsersSettings({ c }: { c: CopySet }) {
  const ko=c.users==="사용자";const[users,setUsers]=useState<UserSummary[]>([]);const[currentUserId,setCurrentUserId]=useState("");const[username,setUsername]=useState("");const[displayName,setDisplayName]=useState("");const[password,setPassword]=useState("");const[role,setRole]=useState<"user"|"admin">("user");const[notice,setNotice]=useState("");const[busy,setBusy]=useState(false);const[managed,setManaged]=useState<UserSummary>();const[defaultQuota,setDefaultQuota]=useState(512);const[defaultUnit,setDefaultUnit]=useState<"MB"|"GB">("MB");
  const syncUsers=(next:UserSummary[])=>{setUsers(next);setManaged(current=>current?next.find(user=>user.id===current.id):undefined);};
  useEffect(()=>{fetch("/api/users").then(async response=>{const body=await response.json();if(!response.ok)throw new Error(body.error);syncUsers(body.users||[]);setCurrentUserId(body.currentUserId||"");const parts=quotaParts(body.defaultStorageQuotaBytes||512*1024**2);setDefaultQuota(parts.value);setDefaultUnit(parts.unit);}).catch(error=>setNotice(error instanceof Error?error.message:"Unable to load users."));},[]);
  async function add(event:FormEvent){event.preventDefault();setBusy(true);setNotice("");try{const response=await fetch("/api/users",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username,displayName:displayName||username,password,role})});const body=await response.json();if(!response.ok)throw new Error(body.error);syncUsers(body.users||[]);setUsername("");setDisplayName("");setPassword("");setNotice(c.saved);}catch(error){setNotice(error instanceof Error?error.message:"Unable to create user.");}finally{setBusy(false);}}
  async function saveDefault(){setBusy(true);setNotice("");try{const bytes=Math.round(defaultQuota*(defaultUnit==="GB"?1024**3:1024**2));const response=await fetch("/api/users/defaults",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({defaultStorageQuotaBytes:bytes})});const body=await response.json();if(!response.ok)throw new Error(body.error);setNotice(c.saved);}catch(error){setNotice(error instanceof Error?error.message:"Unable to save quota.");}finally{setBusy(false);}}
  const used=users.reduce((sum,user)=>sum+user.storageUsedBytes,0),allocated=users.reduce((sum,user)=>sum+user.storageQuotaBytes,0);
  return <div className="settings-section"><SectionTitle icon={<Users size={19}/>} title={c.userManagement} description={c.userManagementDesc}/><div className="user-storage-overview"><div><small>{ko?"전체 저장소 사용량":"Total storage usage"}</small><strong>{storageBytesLabel(used)} <span>/ {storageBytesLabel(allocated)}</span></strong></div><div><small>{ko?"신규 사용자 기본 할당량":"Default quota for new users"}</small><span><input type="number" min={1} step={defaultUnit==="GB"?.25:1} value={defaultQuota} onChange={event=>setDefaultQuota(Number(event.target.value))}/><SelectMenu compact label={ko?"기본 용량 단위":"Default quota unit"} value={defaultUnit} options={[{value:"MB",label:"MB"},{value:"GB",label:"GB"}]} onChange={value=>setDefaultUnit(value as "MB"|"GB")}/><button disabled={busy||defaultQuota<=0} onClick={()=>void saveDefault()}><Check size={15}/>{ko?"기본값 저장":"Save default"}</button></span></div></div>
    <form className="user-create-card" onSubmit={add}><div className="form-grid"><label className="field"><span>{c.username}</span><input value={username} onChange={event=>setUsername(event.target.value)} required/></label><label className="field"><span>{c.displayName}</span><input value={displayName} onChange={event=>setDisplayName(event.target.value)}/></label></div><div className="form-grid"><label className="field"><span>{c.password}</span><input type="password" minLength={8} value={password} onChange={event=>setPassword(event.target.value)} required/></label><label className="field"><span>{c.role}</span><SelectMenu label={c.role} value={role} options={[{value:"user",label:c.standardUser},{value:"admin",label:c.administrator}]} onChange={value=>setRole(value as "user"|"admin")}/></label></div><button className="subtle-action" disabled={busy}><Plus size={16}/>{c.addUser}</button>{notice&&<small className="settings-notice">{notice}</small>}</form>
    <div className="user-list managed-user-list">{users.map(user=><div key={user.id}><span className="avatar-mini">{user.displayName.charAt(0).toUpperCase()}</span><span className="managed-user-name"><strong>{user.displayName}</strong><small>@{user.username} · {user.role}</small></span><span className="managed-user-storage"><strong>{storageBytesLabel(user.storageUsedBytes)}</strong><small>/ {storageBytesLabel(user.storageQuotaBytes)}</small></span><button className="subtle-action" onClick={()=>setManaged(user)}><SlidersHorizontal size={15}/>{ko?"관리":"Manage"}</button></div>)}</div>{managed&&<ManagedUserDialog c={c} user={managed} currentUserId={currentUserId} onUsers={syncUsers} onDeleted={()=>{setManaged(undefined);setNotice(c.userDeleted);}} onClose={()=>setManaged(undefined)}/>}</div>;
}

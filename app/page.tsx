      ...
      <div className="composer-lead" ref={leadRef}>
        <div className="add-menu-wrap" ref={addMenuRef}>
          ...
          {addMenuMounted && <div className={`popover add-menu-popover ${addMenuClosing ? "closing" : ""}`}>
            ...
            <div className="tool-toggle-row"><HardDrive size={17}/><span><strong>{c.storageAccess}</strong><small>{c.storageAccessDesc}</small></span><button type="button" role="switch" aria-label={c.storageAccess} aria-checked={storageAccessEnabled} className={`toggle ${storageAccessEnabled?"on":""}`} onClick={()=>setStorageAccessEnabled(!storageAccessEnabled)}><i/></button></div>
            <p>{c.tools}</p>

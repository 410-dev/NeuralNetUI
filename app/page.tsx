...
import type {
  ...
  ReasoningPreset, StoredAttachment, StoredMessage, StorageFile, Locale, AccountInfo, UserSummary, EnabledTools, ToolEvent, MultipleChoiceQuestion, ToolSettings, AccentPaletteId, AppearancePreferences, LoginAppearance, ConnectionDriver, McpConnection, McpToolInfo, ArtifactDocument,
} from "@/lib/types";
...
export default function Home() {
  ...
  const persistedTool = (key: Exclude<keyof EnabledTools,"mcpConnectionIds"|"mcpToolNames">, set: (value: boolean) => void) => (value: boolean) => {
    set(value);
    ...
    try {
      ...
      const response = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ conversationId: working.id, branchId, assistantMessageId: placeholder.id, revisionGroupId, modelId: working.modelId, reasoningPresetId: working.reasoningPresetId, sendReasoning: options?.sendReasoning ?? sendReasoning,
          tools: options?.tools || { internetSearch: internetSearchEnabled, pageVisit: pageVisitEnabled, browser: browserToolAvailable && browserEnabled, storageAccess:storageAccessEnabled, hostComputer: hostComputerToolAvailable && hostComputerEnabled, currentTime: currentTimeEnabled, location: locationEnabled, multipleChoice: multipleChoiceEnabled,artifact:artifactEnabled, mcpConnectionIds,mcpToolNames },
  ...
  async function sendMessage(event?: FormEvent) {
    ...
    if (isGenerating) {
      ...
      const queued: QueuedPrompt = { id: uid("user"), content: text, attachments: draftAttachments, modelId: requestModel.id, reasoningPresetId: requestPreset?.id, sendReasoning,
        tools: { internetSearch: internetSearchEnabled, pageVisit: pageVisitEnabled, browser: browserEnabled, storageAccess:storageAccessEnabled, hostComputer: hostComputerToolAvailable && hostComputerEnabled, currentTime: currentTimeEnabled, location: locationEnabled, multipleChoice: multipleChoiceEnabled,artifact:artifactEnabled, mcpConnectionIds,mcpToolNames } };
...
function McpApprovalComposer({c,event,onSubmit}:{c:CopySet;event:ToolEvent;onSubmit:(value:unknown)=>Promise<boolean>}){
  ...
  return <section className="choice-composer-card mcp-approval-card" aria-label={c.mcpApprovalTitle}><header><span><Cable size={15}/>{c.mcpApprovalTitle}</span><strong>{String(approval.connectionName||"MCP")}</strong></header><div className="host-approval-body"><p>{c===translations.ko?`“${String(approval.toolName||"")}” 도구 실행을 허용할까요?`:`Allow the “${String(approval.toolName||"")}” tool to run?`}</p><details><summary>{c.toolCall}</summary><StructuredJson value={Object.fromEntries(Object.entries(args).filter(([key])=>key!=="_mcpApproval"))}/></details></div><footer className="host-approval-actions"><button type="button" disabled={submitting} onClick={()=>void submit("approve")}><Check size={15}/>{c.hostApprove}</button><button type="button" disabled={submitting} onClick={()=>void submit("reject")}><X size={15}/>{c.hostReject}</button></footer></section>;
}

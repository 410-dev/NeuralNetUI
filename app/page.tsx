...
function ManagedUserDialog({c,user,currentUserId,onUsers,onDeleted,onClose}:{c:CopySet;user:UserSummary;currentUserId:string;onUsers:(users:UserSummary[])=>void;onDeleted:()=>void;onClose:()=>void}){
  const ko=c.users==="사용자";const initialQuota=quotaParts(user.storageQuotaBytes);const[name,setName]=useState(user.displayName);const[role,setRole]=useState(user.role);const[quota,setQuota]=useState(initialQuota.value);const[unit,setUnit]=useState<"MB"|"GB">(initialQuota.unit);const[notice,setNotice]=useState("");const[busy,setBusy]=useState(false);const[auditView,setAuditView]=useState<"conversations"|"files">();

...
const translations = {
  en: {
    ...
    attachImages: "Attach images or PDFs", loadFromStorage: "Choose files from storage", uploadingImages: "Preparing and uploading files…", removeImage: "Remove attachment", loadEarlier: "Load earlier messages",
  ...
  ko: {
    ...
    attachImages: "이미지 또는 PDF 첨부", loadFromStorage: "저장소에서 파일 불러오기", uploadingImages: "파일 준비 및 업로드 중…", removeImage: "첨부 제거", loadEarlier: "이전 메시지 불러오기",
...
export default function Home() {
  ...
  function attachStoredFiles(files:StorageFile[]){
    setDraftAttachments(current=>{const ids=new Set(current.map(file=>file.id));const available=config.toolSettings.maxAttachmentsPerMessage-current.length;return[...current,...files.filter(file=>!ids.has(file.id)).slice(0,available).map(file=>({...file,fromStorage:true}))];});
      ...
      <div className="composer-lead" ref={leadRef}>
        <div className="add-menu-wrap" ref={addMenuRef}>
          ...
          {addMenuMounted && <div className={`popover add-menu-popover ${addMenuClosing ? "closing" : ""}`}>
            ...
            <button type="button" className="add-menu-action" onClick={() => { onOpenStorage(); setAddMenuOpen(false); }} disabled={attachments.length >= maxAttachments}><HardDrive size={17}/><span><strong>{c.loadFromStorage}</strong></span></button>
            <p>{c.tools}</p>

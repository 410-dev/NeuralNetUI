        ...
        <div className="add-menu-wrap" ref={addMenuRef}>
          ...
          {addMenuOpen && <div className="popover add-menu-popover">
            ...
            <p>{c.tools}</p>
             ...
             <div className="tool-toggle-row"><Monitor size={17} /><span><strong>{c.browserTool}</strong><small>{c.browserToolDesc}</small></span><button type="button" role="switch" aria-checked={browserEnabled} className={`toggle ${browserEnabled ? "on" : ""}`} onClick={() => setBrowserEnabled(!browserEnabled)}><i /></button></div>
...
function ConnectionSettings({ c, draft, setDraft, onDetect, detectingConnectionId }: { c: CopySet; draft: PublicConfig; setDraft: React.Dispatch<React.SetStateAction<PublicConfig>>; onDetect: (connectionId: string) => void; detectingConnectionId: string }) {
  ...
  function replaceConnections(connections: PublicConfig["connections"]) { setDraft((current) => ({ ...current, connections, models: resolveConnectionModels(connections as ConnectionConfig[], current.models.filter((model) => model.isAlias), current.models.map((model) => model.id)) })); }
  ...
  return <div className="settings-section wide"><SectionTitle icon={<Server size={19} />} title={c.serverTitle} description={c.serverDesc} />
    ...
    <div className="split-model-editor connection-editor">
      ...
      <div className="model-editor-pane">{activeConnection ? <div className="editor-card connection-card">{(() => { const detecting = detectingConnectionId === activeConnection.id; return <>
        ...
        <label className="field"><span>{c.apiKey}</span><div className="field-with-icon"><KeyRound size={16} /><input type="password" value={activeConnection.apiKey} onChange={(event) => patchConnection(activeConnection.id, { apiKey: event.target.value })} placeholder={activeConnection.hasApiKey ? c.savedKey : activeConnection.driver === "lmstudio" ? "Optional" : c.requiredKey} /></div><small>{c.apiKeyHelp}</small></label>

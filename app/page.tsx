...
import type {
  ...
  ReasoningPreset, StoredAttachment, StoredMessage, Locale, AccountInfo, UserSummary, EnabledTools, ToolEvent, MultipleChoiceQuestion, ToolSettings, AccentPaletteId, AppearancePreferences, LoginAppearance, ConnectionDriver,
} from "@/lib/types";
...
const emptyConfig: PublicConfig = {
  ...
  userStorageSettings: { defaultQuotaBytes: 512 * 1024 * 1024, trashRetentionDays: 60 },
...
function ManagedUserDialog({c,user,currentUserId,currentUserCanAudit,currentUserRole,onUsers,onDeleted,onClose}:{c:CopySet;user:UserSummary;currentUserId:string;currentUserCanAudit:boolean;currentUserRole:string;onUsers:(users:UserSummary[])=>void;onDeleted:()=>void;onClose:()=>void}){
  const ko=c.users==="사용자";const initialQuota=quotaParts(user.storageQuotaBytes),initialTrashQuota=quotaParts(user.trashQuotaBytes);const[name,setName]=useState(user.displayName);const[role,setRole]=useState(user.role);const[auditEnabled,setAuditEnabled]=useState(user.auditEnabled);const[quota,setQuota]=useState(initialQuota.value);const[unit,setUnit]=useState<"MB"|"GB">(initialQuota.unit);const[trashQuota,setTrashQuota]=useState(initialTrashQuota.value);const[trashUnit,setTrashUnit]=useState<"MB"|"GB">(initialTrashQuota.unit);const[notice,setNotice]=useState("");const[busy,setBusy]=useState(false);const[auditView,setAuditView]=useState<"conversations"|"files">();
  async function save(){setBusy(true);setNotice("");try{const storageQuotaBytes=Math.round(quota*(unit==="GB"?1024**3:1024**2)),trashQuotaBytes=Math.round(trashQuota*(trashUnit==="GB"?1024**3:1024**2));const payload=user.id===currentUserId?{storageQuotaBytes,trashQuotaBytes}:{displayName:name,role:user.role==="superadmin"?undefined:role,storageQuotaBytes,trashQuotaBytes,...(currentUserRole==="superadmin"?{auditEnabled:role==="admin"&&auditEnabled}:{})};const response=await fetch(`/api/users/${encodeURIComponent(user.id)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});const body=await response.json();if(!response.ok)throw new Error(body.error);onUsers(body.users||[]);setNotice(c.saved);}catch(error){setNotice(error instanceof Error?error.message:"Unable to update user.");}finally{setBusy(false);}}
...
function UsersSettings({ c }: { c: CopySet }) {
  ...
  async function saveDefault(){setBusy(true);setNotice("");try{const bytes=Math.round(defaultQuota*(defaultUnit==="GB"?1024**3:1024**2));const response=await fetch("/api/users/defaults",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({defaultStorageQuotaBytes:bytes,trashRetentionDays:retentionDays})});const body=await response.json();if(!response.ok)throw new Error(body.error);setNotice(c.saved);}catch(error){setNotice(error instanceof Error?error.message:"Unable to save quota.");}finally{setBusy(false);}}
  const used=users.reduce((sum,user)=>sum+user.storageUsedBytes,0),allocated=users.reduce((sum,user)=>sum+user.storageQuotaBytes,0),trashUsed=users.reduce((sum,user)=>sum+user.trashUsedBytes,0),trashAllocated=users.reduce((sum,user)=>sum+user.trashQuotaBytes,0);
  return <div className="settings-section"><SectionTitle icon={<Users size={19}/>} title={c.userManagement} description={c.userManagementDesc}/><div className="user-storage-overview"><div><small>{ko?"전체 저장소 사용량":"Total storage usage"}</small><strong>{storageBytesLabel(used+trashUsed)} <span>/ {storageBytesLabel(allocated+trashAllocated)}</span></strong><div className="aggregate-storage-bars"><StorageUsageMeter used={used} total={allocated} label={ko?"실제 저장소":"Active storage"} ko={ko}/><StorageUsageMeter used={trashUsed} total={trashAllocated} label={ko?"휴지통 저장소":"Trash storage"} ko={ko}/></div></div><div><small>{ko?"신규 사용자 기본 할당량":"Default quota for new users"}</small><span><input type="number" min={1} step={defaultUnit==="GB"?.25:1} value={defaultQuota} onChange={event=>setDefaultQuota(Number(event.target.value))}/><SelectMenu compact label={ko?"기본 용량 단위":"Default quota unit"} value={defaultUnit} options={[{value:"MB",label:"MB"},{value:"GB",label:"GB"}]} onChange={value=>setDefaultUnit(value as "MB"|"GB")}/><button disabled={busy||defaultQuota<=0||retentionDays<1||retentionDays>60} onClick={()=>void saveDefault()}><Check size={15}/>{ko?"기본값 저장":"Save default"}</button></span><label className="retention-days"><span>{ko?"삭제 보존 기간 (최대 60일)":"Deleted-item retention (up to 60 days)"}</span><input type="number" min={1} max={60} value={retentionDays} onChange={event=>setRetentionDays(Number(event.target.value))}/></label></div></div>
    ...
    <div className="user-list managed-user-list">{users.map(user=><div key={user.id}><span className="avatar-mini">{user.displayName.charAt(0).toUpperCase()}</span><span className="managed-user-name"><strong>{user.displayName}</strong><small>@{user.username} · {user.role}{user.canAudit?` · ${ko?"감사":"audit"}`:""}</small></span><span className="managed-user-storage"><strong>{storageBytesLabel(user.storageUsedBytes+user.trashUsedBytes)}</strong><small>/ {storageBytesLabel(user.storageQuotaBytes+user.trashQuotaBytes)}</small></span><button className="subtle-action" onClick={()=>setManaged(user)}><SlidersHorizontal size={15}/>{ko?"관리":"Manage"}</button></div>)}</div>{managed&&<ManagedUserDialog c={c} user={managed} currentUserId={currentUserId} currentUserCanAudit={currentUserCanAudit} currentUserRole={currentUserRole} onUsers={syncUsers} onDeleted={()=>{setManaged(undefined);setNotice(c.userDeleted);}} onClose={()=>setManaged(undefined)}/>}</div>;
}

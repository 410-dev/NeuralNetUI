...
export function HistorySearch({ko,onClose,onSelect}:{ko:boolean;onClose:()=>void;onSelect:(id:string,branch:string)=>void}) {
  ...
  return createPortal(<div className="harness-modal-layer" ref={ref} tabIndex={-1} role="dialog" aria-modal="true" aria-label={ko?"채팅 기록 검색":"Search chat history"}><button className="settings-backdrop" tabIndex={-1} onClick={onClose} aria-label={ko?"닫기":"Close"}/><section className="harness-dialog"><header><h2>{ko?"채팅 기록 검색":"Search chat history"}</h2><button onClick={onClose} aria-label={ko?"닫기":"Close"}><X size={20}/></button></header><input data-autofocus aria-label={ko?"검색어":"Search query"} placeholder={ko?"모든 분기의 채팅 원문 검색…":"Search original messages in all branches…"} value={query} maxLength={200} onChange={e=>setQuery(e.target.value)}/><p role="status">{status}</p><div className="search-results">{results.map(r=><button key={`${r.id}:${r.branchId}`} onClick={()=>onSelect(r.id,r.branchId)}><strong>{r.title}{r.otherBranch&&<GitBranch size={16} aria-label={ko?"다른 분기에서 발견":"Match in another branch"}/>}</strong><small>{r.branchName}</small><p>{r.content}</p></button>)}</div></section></div>,document.body);
}

...
export async function GET(request:Request){try{const user=requireUser(request);return Response.json(usageStatus(user.id,new Date(),true));}catch(error){return authErrorResponse(error);}}
